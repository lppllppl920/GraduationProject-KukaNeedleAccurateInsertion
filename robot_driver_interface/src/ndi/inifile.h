...
#ifndef ROBOT_DRIVER_INTERFACE_SRC_NDI_INIFILE_H_
#define ROBOT_DRIVER_INTERFACE_SRC_NDI_INIFILE_H_
...
#ifndef _WIN32
				return strcasecmp(s1->m_sKeyName.c_str(),
						s2->m_sKeyName.c_str()) < 0;
#else
...
#ifndef _WIN32
			return strcasecmp(s1->m_sSectionName.c_str(),
					s2->m_sSectionName.c_str()) < 0;
#else
...
public:
	...
	// Returns a KeyValue at a certain section
	std::string GetKeyValue(const std::string& sSection,
			const std::string& sKey) const;
	// Sets a KeyValuePair at a certain section
	void SetKeyValue(const std::string& sSection, const std::string& sKey,
	...
	bool RenameSection(const std::string& sSectionName,
	...
	bool RenameKey(const std::string& sSectionName, const std::string& sKeyName,
			const std::string& sNewKeyName);
private:
...
#ifndef _WIN32
				return wcscasecmp(s1->m_sKeyName.c_str(),
						s2->m_sKeyName.c_str()) < 0;
#else
...
#ifndef _WIN32
			return wcscasecmp(s1->m_sSectionName.c_str(),
					s2->m_sSectionName.c_str()) < 0;
#else
...
public:
	...
	// Returns a KeyValue at a certain section
	std::wstring GetKeyValue(const std::wstring& sSection,
			const std::wstring& sKey) const;
	// Sets a KeyValuePair at a certain section
	void SetKeyValue(const std::wstring& sSection, const std::wstring& sKey,
			const std::wstring& sValue);
	// Renames an existing section returns true on success, false if the section didn't exist or there was another section with the same sNewSectionName
	bool RenameSection(const std::wstring& sSectionName,
			const std::wstring& sNewSectionName);
	// Renames an existing key returns true on success, false if the key didn't exist or there was another section with the same sNewSectionName
	bool RenameKey(const std::wstring& sSectionName,
...
#endif /* ROBOT_DRIVER_INTERFACE_SRC_NDI_INIFILE_H_ */
