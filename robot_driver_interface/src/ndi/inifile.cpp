#ifndef ROBOT_DRIVER_INTERFACE_SRC_NDI_INIFILE_CPP_
#define ROBOT_DRIVER_INTERFACE_SRC_NDI_INIFILE_CPP_
...
void CIniFileA::Save(std::ostream& output) {
	...
	for (SecIndexA::iterator itr = m_sections.begin(); itr != m_sections.end();
			++itr) {
		...
		for (KeyIndexA::iterator klitr = (*itr)->m_keys.begin();
				klitr != (*itr)->m_keys.end(); ++klitr) {
			std::string sKey = (*klitr)->GetKeyName() + "="
					+ (*klitr)->GetValue();
#ifdef _CINIFILE_DEBUG
...
void CIniFileA::Load(std::istream& input, bool bMerge) {
	...
	while (std::getline(input, sRead)) {
		...
		if (!sRead.empty()) {
			unsigned int nType =
					(sRead.find_first_of("[") == 0
							&& (sRead[sRead.find_last_not_of(" \t\r\n")] == ']')) ?
							SECTION : OTHER;
			nType = ((nType == OTHER)
					&& (sRead.find_first_of("=") != std::string::npos
							&& sRead.find_first_of("=") > 0)) ? KEY : nType;
			nType = ((nType == OTHER) && (sRead.find_first_of("#") == 0)) ?
					COMMENT : nType;
...
std::string CIniFileA::GetKeyValue(const std::string& sSection,
...
void CIniFileA::SetKeyValue(const std::string& sSection,
...
bool CIniFileA::RenameSection(const std::string& sSectionName,
		const std::string& sNewSectionName) {
#ifdef _CINIFILE_DEBUG
...
bool CIniFileA::RenameKey(const std::string& sSectionName,
		const std::string& sKeyName, const std::string& sNewKeyName) {
#ifdef _CINIFILE_DEBUG
...
SecIndexA::const_iterator CIniFileA::_find_sec(
		const std::string& sSection) const {
...
KeyIndexA::const_iterator CIniSectionA::_find_key(
		const std::string& sKey) const {
#ifdef _CINIFILE_DEBUG
...
void CIniFileW::Save(std::wostream& output) {
	...
	for (SecIndexW::iterator itr = m_sections.begin(); itr != m_sections.end();
			++itr) {
		...
		for (KeyIndexW::iterator klitr = (*itr)->m_keys.begin();
				klitr != (*itr)->m_keys.end(); ++klitr) {
			std::wstring sKey = (*klitr)->GetKeyName() + L"="
					+ (*klitr)->GetValue();
#ifdef _CINIFILE_DEBUG
...
void CIniFileW::Load(std::wistream& input, bool bMerge) {
	...
	while (std::getline(input, sRead)) {
		...
		if (!sRead.empty()) {
			unsigned int nType =
					(sRead.find_first_of(L"[") == 0
							&& (sRead[sRead.find_last_not_of(L" \t\r\n")]
									== L']')) ? SECTION : OTHER;
			nType = ((nType == OTHER)
					&& (sRead.find_first_of(L"=") != std::wstring::npos
							&& sRead.find_first_of(L"=") > 0)) ? KEY : nType;
			nType = ((nType == OTHER) && (sRead.find_first_of(L"#") == 0)) ?
					COMMENT : nType;
...
std::wstring CIniFileW::GetKeyValue(const std::wstring& sSection,
...
void CIniFileW::SetKeyValue(const std::wstring& sSection,
		const std::wstring& sKey, const std::wstring& sValue) {
#ifdef _CINIFILE_DEBUG
...
bool CIniFileW::RenameSection(const std::wstring& sSectionName,
		const std::wstring& sNewSectionName) {
#ifdef _CINIFILE_DEBUG
...
bool CIniFileW::RenameKey(const std::wstring& sSectionName,
		const std::wstring& sKeyName, const std::wstring& sNewKeyName) {
#ifdef _CINIFILE_DEBUG
...
// Returns a constant iterator to a section by name, string is not trimmed
SecIndexW::const_iterator CIniFileW::_find_sec(
		const std::wstring& sSection) const {
...
CIniSectionW::CIniSectionW(CIniFileW* pIniFile,
		const std::wstring& sSectionName) :
		m_pIniFile(pIniFile), m_sSectionName(sSectionName) {
#ifdef _CINIFILE_DEBUG
...
// Returns a constant iterator to a key by name, string is not trimmed
KeyIndexW::const_iterator CIniSectionW::_find_key(
		const std::wstring& sKey) const {
#ifdef _CINIFILE_DEBUG
...
#endif /* ROBOT_DRIVER_INTERFACE_SRC_NDI_INIFILE_CPP_ */
